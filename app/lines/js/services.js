    ...
    link: function(scope, iElement, iAttrs, ctrls) {
        scope.showModal = function() {
            if (!scope.modalContent) {
                $http.get('md/'+scope.modalMd+'.md?ver=20170503c', {responseType: 'text', cache: true
                }).success(function(data) {
                    scope.modalContent = data;
                });
            }
            scope.modalInstance = $modal.open({
                templateUrl: 'partials/modal.html?ver=20170503c',
                scope: scope,
            });
...
services.factory('lineTableVars', function lineTableVarsFactory() {
  ...
  f.assays = [
              ...
              {short: 'wgs', long: 'Whole genome sequencing'},
  ...
  f.fields = [
        ...
        {visible: false,  sortable: false, selectable: false, esName: 'bioSamplesAccession', label: 'Biosample'},
    ...
    for (var i=0; i<f.fields.length; i++) {
        ...
        if (field.visible || field.selectable) {
            field.th = 
              ...
              : '<th>'+field.label+'</th>'
            var hitStr = 'hit['+i+']';
            if (field.esName == 'assays.name') {console.log(hit[20][$index])};
